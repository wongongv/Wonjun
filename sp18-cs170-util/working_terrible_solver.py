...
def solve(list_of_kingdom_names, starting_kingdom, adjacency_matrix, params=[]):
    ...
    P = list(G.nodes())
    P.remove(start)
    closed_walk.append(starting_kingdom)
    for node in P:
        inter = nx.dijkstra_path(G, start, node)
        print(inter)
        for i in range(1, len(inter)):
            closed_walk.append(list_of_kingdom_names[inter[i]])
            conquered_kingdoms.append(list_of_kingdom_names[inter[i]])
        start = node

    path_back = nx.dijkstra_path(G, start, list_of_kingdom_names.index(starting_kingdom))
    for i in range(1, len(path_back)):
        closed_walk.append(list_of_kingdom_names[path_back[i]])
    print("DOG" + str(closed_walk))
