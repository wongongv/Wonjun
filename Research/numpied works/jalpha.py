...
class jalpha:
	...
	def update(self):
		...
		for i in self.alphas_pre:
			self.rho_avg_pre = np.append(self.rho_avg_pre,[self.cal_rho(self.js(i,glo_var.beta))])
		self.rho_avg_post = []
		for i in self.alphas_post:
			self.rho_avg_post =np.append(self.rho_avg_post,[self.cal_rho(self.js(i,glo_var.beta))])

		self.num=30

		# may be not using below?
	...
	def cal_rho(self,jval):
		...
		for x,y in self.rhointercal:
			...
			self.rho_l = np.append(self.rho_l,[x - self.inter_y]) 
			self.rho_r = np.append(self.rho_r,[x + self.inter_y])
	...
	def check_two_mins(self):
		...
		if num > 1:
			for j in range(num - 1):
				val = np.max(self.lambdas_ys[self.minlocation[j]:self.minlocation[j+1]])
				self.maxlocation += [np.nonzero(self.lambdas_ys[self.minlocation[j]:] == val)[0][0] + self.minlocation[j]]
		return num


	def plot_scat(self,steps):
		self.num_mins = self.check_two_mins()
		self.scat_ys = np.array([])
		self.scat_xs = np.array([])
		if self.region == 3:
			if self.num_mins > 1:
				self.index1 = self.minlocation[0]*self.xperlambdas
				self.scat_xs = np.append(self.scat_xs, self.getscatarray(self.rh.lambdas_xval[:self.index1],self.xperlambdas))
				self.scat_ys = np.append(self.scat_ys, self.getscatarray(self.rho_r[:self.index1],self.xperlambdas))
				for i in range(1, self.num_mins):
					...
					self.scat_xs = np.append(self.scat_xs, self.getscatarray(self.rh.lambdas_xval[self.index1:self.indexmax],self.xperlambdas))
					self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_l[self.index1:self.indexmax],self.xperlambdas))
					self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.rh.lambdas_xval[self.indexmax:self.index2],self.xperlambdas))
					self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_r[self.indexmax:self.index2],self.xperlambdas))
					...
					for j in range(self.dist):
						self.scat_xs = np.append(self.scat_xs , [self.rh.lambdas_xval[self.indexmax]])
						self.scat_ys = np.append(self.scat_ys , [lb + j*difference/self.dist])
				self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.rh.lambdas_xval[self.index2:],self.xperlambdas))
				self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_l[self.index2:],self.xperlambdas))
				
				self.scat_xs = np.append(self.scat_xs , [self.rh.lambdas_xval[-1]])
				self.scat_ys = np.append(self.scat_ys , [self.rho_l[-1]])
			else :
				self.index = self.minlocation[0]*self.xperlambdas
				self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.rh.lambdas_xval[:self.index],self.xperlambdas) + self.getscatarray(self.rh.lambdas_xval[self.index:],self.xperlambdas))
				self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_r[:self.index],self.xperlambdas) + self.getscatarray(self.rho_l[self.index:],self.xperlambdas))

				self.scat_xs = np.append(self.scat_xs , [self.rh.lambdas_xval[-1]])
				self.scat_ys = np.append(self.scat_ys , [self.rho_l[-1]])
	#  what if num_mins is 3?
		elif self.region == 2:
			self.scat_xs = self.getscatarray(self.rh.lambdas_xval,self.xperlambdas)
			self.scat_ys = self.getscatarray(self.rho_r,self.xperlambdas)

			self.scat_xs = np.append(self.scat_xs ,[self.rh.lambdas_xval[-1]])
			self.scat_ys = np.append(self.scat_ys , [self.rho_r[-1]])
		else:
			self.scat_xs = self.getscatarray(self.rh.lambdas_xval,self.xperlambdas)
	...
	def value_declaration(self):
		self.lambdas_xs = glo_var.lambdas[:,0]
		self.lambdas_ys = glo_var.lambdas[:,1]
		self.lambda_min = np.min(self.lambdas_ys)
	...
	def plot_sum_rho(self):
		...
		for i in self.domain:
			...
			self.rho_sum = np.append(self.rho_sum,[self.basic_1 + self.j_inter*self.basic_2 +pow(-1,self.region == 1) * self.inter_sum/glo_var.lambdas_degree])
			self.inter_sum = 0
