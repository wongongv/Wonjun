...
# calculating transition line
	...
	def calculation(self):
		...
		self.lamb_func = np.array([])
		for i in range(glo_var.lambdas_degree - 1):
			print("working")
			self.lamb_func = np.append(self.lamb_func,[lambda x : ((self.lambdas_ys[i + 1] - self.lambdas_ys[i])/(self.lambdas_xs[i + 1] - self.lambdas_xs[i])) * ( x - self.lambdas_xs[i]) + self.lambdas_ys[i]])
		self.xperlambdas = 1 if round(149/glo_var.lambdas_degree) == 0 else round(149/glo_var.lambdas_degree)
		

		self.lambdas_yval = np.array([])
		self.lambdas_xval = np.array([])
		for i in range(glo_var.lambdas_degree - 1):
			for j in np.linspace(self.lambdas_xs[i],self.lambdas_xs[i + 1], self.xperlambdas):
				self.lambdas_xval = np.append(self.lambdas_xval,j)
				self.lambdas_yval = np.append(self.lambdas_yval,self.lamb_func[i](j))
				# self.lambdas_xval+= [j]
	...
	def value_declaration(self):
		...
		self.lambda_max=np.max(self.lambdas_ys)
		self.scat_step = 3
	...
	def plot_scat(self,steps):
		...
		if self.region == 3:
			if self.num_mins > 1:
				...
				self.scat_xs = np.append(self.scat_xs, self.getscatarray(self.lambdas_xval[:self.index1],self.xperlambdas))
				...
				for i in range(1, self.num_mins):
					...
					self.scat_xs = np.append(self.scat_xs, self.getscatarray(self.lambdas_xval[self.index1:self.indexmax],self.xperlambdas))
					self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_l[self.index1:self.indexmax],self.xperlambdas))
					self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.lambdas_xval[self.indexmax:self.index2],self.xperlambdas))
					...
					for j in range(self.dist):
						self.scat_xs = np.append(self.scat_xs , [self.lambdas_xval[self.indexmax]])
						self.scat_ys = np.append(self.scat_ys , [lb + j*difference/self.dist])
				self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.lambdas_xval[self.index2:],self.xperlambdas))
				self.scat_ys = np.append(self.scat_ys , self.getscatarray(self.rho_l[self.index2:],self.xperlambdas))
				
				self.scat_xs = np.append(self.scat_xs , [self.lambdas_xval[-1]])
				self.scat_ys = np.append(self.scat_ys , [self.rho_l[-1]])
			else :
				self.index = self.minlocation[0]*self.xperlambdas
				self.scat_xs = np.append(self.scat_xs , self.getscatarray(self.lambdas_xval[:self.index],self.xperlambdas) + self.getscatarray(self.lambdas_xval[self.index:],self.xperlambdas))
