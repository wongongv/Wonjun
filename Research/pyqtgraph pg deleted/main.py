...
import numpy as np
from pyqtgraph import PlotWidget, setConfigOption, exporters, setConfigOptions
import glo_var
import lamb_pol
import slider
import rho
...
# for dock
	def __init__(self,parent=None):
		...
		setConfigOption('background', QtGui.QColor(215,214,213,255))
...
# all those have different relative positions so made bunch of functions, not one.
	...
	def exportimg(self,img,path):
		self.img = exporters.ImageExporter(img)
	...
	def maketoolbar(self):
		...
		home = QtGui.QAction(QtGui.QIcon(QtGui.QApplication.style().standardIcon(QtGui.QStyle.SP_ComputerIcon)),'Grid layout 1',self)
		home.triggered.connect(self.grid_view_1)
		self.toolbar.addAction(home)

		grid2 = QtGui.QAction(QtGui.QIcon(QtGui.QApplication.style().standardIcon(QtGui.QStyle.SP_ComputerIcon)),'Grid layout 1',self)
		grid2.triggered.connect(self.grid_view_2)
		self.toolbar.addAction(grid2)
		...
		info.triggered.connect(self.about_info)
