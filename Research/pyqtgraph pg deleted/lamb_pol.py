...
import numpy as np
from pyqtgraph import ScatterPlotItem, mkPen, mkBrush, PlotCurveItem
import glo_var
from pyqtgraph.widgets import SpinBox


class myscat(ScatterPlotItem):
...
class Menu(QtGui.QMenu):
	def __init__(self, item, point):
		...
		self.fracPosSpin = SpinBox.SpinBox()
...
# I didnt use it. Think about it.
		...
		self.set_range()
		self.sp = myscat(size = 7, pen = mkPen(None), brush=mkBrush(100,200,200), symbolPen='w')
		self.lastClicked=[]
