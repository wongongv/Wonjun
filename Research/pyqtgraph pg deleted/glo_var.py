...
from pyqtgraph import pixmaps, LegendItem, LabelItem, GraphicsWidget, mkPen, mkBrush, ScatterPlotItem, PlotDataItem, PlotWidget, SignalProxy
...
class MyItem(GraphicsWidget):
	...
	def paint(self, p, *args):
		...
		if opts.get('fillLevel',None) is not None and opts.get('fillBrush',None) is not None:
			p.setBrush(mkBrush(opts['fillBrush']))
			p.setPen(mkPen(None))
			p.drawPolygon(QtGui.QPolygonF([QtCore.QPointF(2,18), QtCore.QPointF(18,2), QtCore.QPointF(18,18)]))
		
		if not isinstance(self.item, ScatterPlotItem):
			p.setPen(mkPen(opts['pen']))
			p.drawLine(2, 2, 18, 2)
