...
class phase:
	def __init__(self, dphase):
		...
		self.dphase = dphase
		self.p5main = glo_var.MyPW(x="\u03b1",y1="\u03b2",set_range = self.set_range, num = 5, set_range_byspbox = self.set_range_byspbox)
		...
		self.range = [[0,2*glo_var.alpha_star],[0,2*glo_var.beta_star]]
		self.p5main.receive_range(self.range)
		self.set_range()
	...
	def set_range(self):
		...
		self.range[0] = [0,2*glo_var.alpha_star]
		self.range[1] = [0,2*glo_var.beta_star]
		self.p5main.set_spbox_value()
